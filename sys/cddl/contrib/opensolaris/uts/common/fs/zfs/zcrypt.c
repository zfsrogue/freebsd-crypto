	...
	caddr_t *wkeybuf, size_t *wkeylen, uint64_t wcrypt)
{
	...
	random_get_bytes((uchar_t *)wkeyphys->zkp_kiv, ivlen);
	...
	/* Look into why FreeBSD random returns "1" */
	random_get_bytes((uchar_t *)wkeyphys->zkp_miv, ivlen);
...
zcrypt_key_gen(int crypt)
{
	...
	random_get_bytes((uchar_t *)genkeybuf, genkeylen);
